...
function kits.get_by_name(kit_name, cwd)
  ...
  if config then
    for _, item in ipairs(config) do
      local name = item.name
      if name == kit_name then return item end
    end
  end
  return nil
end
...
function kits.build_env_and_args(kit_name, escape, cwd)
  ...
  if kit.environmentSetupScript then
    env_script = kit.environmentSetupScript -- vim.print(env_script)
  end
  ...
  if kit.host_architecture then table.insert(args, "-T host=" .. kit.host_architecture) end
  if kit.target_architecture then table.insert(args, "-A " .. kit.target_architecture) end
  if kit.toolchainFile then add_args { "-DCMAKE_TOOLCHAIN_FILE:FILEPATH=" .. kit.toolchainFile } end
